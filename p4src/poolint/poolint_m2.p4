    ...
    action ecmp_group(bit<8> group_size) {
        ...
        hash(meta.ecmp_select, HashAlgorithm.crc16, (bit<16>)0,
             { hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.ipv4.protocol,
               meta.ecmp_l4_src, meta.ecmp_l4_dst },
             (bit<16>)group_size);
    }
...
control PoolEgress(inout headers hdr,
    ...
    apply {
        if (meta.pool_active == 1) {
            ...
            /* gate #0 debug: source stamps H(FAIL,0) for its own port_uid;
             * spines stamp their swid */
            ...
            if (meta.switch_id >= 100) {
                hdr.poolint_dbg.dbg_spine = (bit<8>)meta.switch_id;
            }
        }
    }
}
